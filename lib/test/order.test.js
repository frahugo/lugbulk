...
describe("order", function () {
  ...
  context("loading order", function () {
    ...
    it("should load elements", function () {
      ...
      element.should.have.property("orderedQuantity").and.equal(2750);
    ...
    it("should warn when ordered quantity does not match total quantity", function () {
      const warnings = [];
      const originalWarn = console.warn;
      console.warn = (msg) => warnings.push(msg);

      const order = new Order();
      order.load(path.resolve(path.join(__dirname, "fixtures/order.csv")));
